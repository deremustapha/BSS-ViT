# -*- coding: utf-8 -*-
"""
Created on Sun Jan  8 08:11:16 2023
...
def get_experiment_data(path, subjects, sessions, signal_type, input_type, channels, low_cut, high_cut,
    ...
    if type(sessions) is list:
        data_1, label_1 = load_single_subject_session(path, subject=subjects, session=sessions[0])
        data_2, label_2 = load_single_subject_session(path, subject=subjects, session=sessions[1])
        
        data = np.concatenate((data_1, data_2))
        label = np.concatenate((label_1, label_2))
    
    else:
        data, label = load_single_subject_session(path, subject=subjects, session=sessions)

    
    win_data, win_label =  aggregate_data_label(data, label, no_channels=channels, signal_type=signal_type,
                        l_cutoff=low_cut, h_cutoff=high_cut, order=order, window_size=window_size, overlap_per=overlap, 
...
def vit_gesture_classification(input_shape, dims, patch_size, no_patches, no_transformer_layers,
...
def CNN_gesture_classification(input_shape, dims, patch_size, no_patches, no_transformer_layers,
                               no_heads, transformer_units, mlp_head_units, no_classes,
                               stochastic_depth,stochastic_depth_rate):
...
def CNN_LSTM_gesture_classification(input_shape, dims, patch_size, no_patches, no_transformer_layers,
                               no_heads, transformer_units, mlp_head_units, no_classes,
                               stochastic_depth,stochastic_depth_rate):
...
def LSTM_gesture_classification(input_shape, dims, patch_size, no_patches, no_transformer_layers,
                               no_heads, transformer_units, mlp_head_units, no_classes,
                               stochastic_depth,stochastic_depth_rate):
...
def run_experiment(n_folds, n_batches, n_epochs, start_subject, total_subject, session, path, 
                       input_type,n_channels, window_size, extend, extend_size, center, whiten, ratio,
                       noise_db, std,
    ...
    for s in range(start_subject, (total_subject+1)):
        ...
        X_train, y_train, X_test, y_test = get_experiment_data(path, subjects=s, sessions=session,
                                                      ...
                                                      order=6, window_size=window_size, overlap=50, fs=2048, 
        ...
        for train, test in kfold.split(X_train, y_train):
            ...
            #model = CNN_gesture_classification(input_shape, dims, patch_size, n_patches,
            #                      n_transformer_layers, n_heads, transformer_units,
            #                    mlp_head_units, n_classes,stochastic_depth,stochastic_depth_rate)
            #cnn_gesture_classification
            #CNN_LSTM_gesture_classification
            
            #model = CNN_LSTM_gesture_classification(input_shape, dims, patch_size, n_patches,
            #                      n_transformer_layers, n_heads, transformer_units,
            #                      mlp_head_units, n_classes,stochastic_depth,stochastic_depth_rate)
            
            
            #model = LSTM_gesture_classification(input_shape, dims, patch_size, n_patches,
            #                      n_transformer_layers, n_heads, transformer_units,
            #                      mlp_head_units, n_classes,stochastic_depth,stochastic_depth_rate)
        ...
        save_path =  'Experiment_'+ str(type_of_experiment)+'_'+input_type+'_'+str(start_subject)+ '_to_'+str(total_subject)+'.csv'
